...
function M.setup_dependencies(prob_dir)
	...
	local lang = language_map[language]
	local deps = dependencies[lang]
...
function M.save_solution_file(prob_dir, snippets, problem_data)
	...
	local extension = extension_map[C.default_language]
	...
	if f then
		...
		-- Map default_language slug → header/dependency key
		local language_map = {
			["cpp"] = "cpp",
			["python"] = "python",
			["java"] = "java",
		...
		local lang = language_map[C.default_language]
